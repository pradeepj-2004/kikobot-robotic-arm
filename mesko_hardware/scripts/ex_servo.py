...
def main():
    ...
    def enable():
        while not node.start_service.wait_for_service(timeout_sec=1.0):
            node.get_logger().info("Waiting for /servo_node/start_servo service to be available...")
        
        request = Trigger.Request()
        future = node.start_service.call_async(request)
        rclpy.spin_until_future_complete(node, future)

        if future.result() is not None:
            node.get_logger().info("Servo started successfully.")
        else:
            node.get_logger().error("Failed to start servo.")
    ...
    def servo_circular_motion():
            ...
            node.twist_msg.twist.linear.y = 0.0
            node.twist_msg.twist.linear.z = 0.1
    ...
    enable()
    node.create_timer(0.23, servo_circular_motion)
